// restaurar-selecoes-canceladas.js
// Script para restaurar as 3 seleções que foram canceladas automaticamente pelo sync
...
async function restaurarSelecoes() {
    try {
        console.log('🔧 RESTAURAÇÃO DE SELEÇÕES CANCELADAS AUTOMATICAMENTE');
        ...
        for (const config of SELECOES_PARA_RESTAURAR) {
            ...
            for (const fotoName of config.fotos) {
                ...
                try {
                    ...
                    if (categoria) {
                        // Usar preço base por enquanto (vamos recalcular depois)
            ...
            // 4. Recalcular preços (usar lógica simplificada)
            ...
            // Buscar preços corretos por categoria e tier
            for (const item of selecao.items) {
                try {
                    const categoria = await PhotoCategory.findOne({ 
                        $or: [
            ...
            // 7. Adicionar log de restauração
            if (selecao.addMovementLog) {
                selecao.addMovementLog(
                    'restored',
                    `✅ Seleção restaurada manualmente. ${fotosParaAdicionar.length} foto(s) recuperada(s). Novo total: $${totalValue.toFixed(2)}`,
                    true,
                    null,
                    { 
                        restoredPhotos: config.fotos,
                        restoredBy: 'admin-script',
                        reason: 'Seleção foi cancelada automaticamente pelo sync de forma incorreta'
                    }
                );
            } else {
                ...
                selecao.movementLog.push({
                    action: 'restored',
                    timestamp: new Date(),
                    details: `✅ Seleção restaurada manualmente. ${fotosParaAdicionar.length} foto(s) recuperada(s). Novo total: $${totalValue.toFixed(2)}`,
                    isSystem: true,
                    metadata: { 
                        restoredPhotos: config.fotos,
                        restoredBy: 'admin-script'
            ...
            // 8. Atualizar fotos no MongoDB para vincular à seleção
