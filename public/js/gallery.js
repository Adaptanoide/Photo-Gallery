...
class DashboardNavigation {
  loadCategory(categoryKey) {
    ...
    // Mapear nome das categorias
    const categoryNames = {
      'brazil-best-sellers': 'Brazil Best Sellers',
      'brazil-top-selected': 'Brazil Top Selected',
      'calfskins': 'Calfskins',
      'colombian-cowhides': 'Colombian Cowhides',
      'rodeo-rugs': 'Rodeo Rugs',
      'sheepskins': 'Sheepskins'
    };

    const categoryName = categoryNames[categoryKey] || categoryKey;

    // Usar breadcrumb navigation
    if (window.breadcrumbNavigation) {
      window.breadcrumbNavigation.navigateToMainCategory(categoryKey, categoryName);
    } else {
      // Fallback para navegação antiga
